...
def get_substitution_matrix():
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..')) #get the project root.
    matrix_path = os.path.join(project_root, 'data', 'blosum62.txt')
    return utils.import_substitution_matrix(matrix_path)

SUBSTITUTION_MATRIX = get_substitution_matrix()
...
def generate_protease_labels(
    ...
    for protease in proteases:
        family = merops.retrieve_protease_family_code(protease)
...
def generate_non_exact_protease_pattern_matrix(
        patterns,
        protease_patterns,
        protease_labels
    ):
    ...
    for pattern in patterns.pattern_list:
        ...
        for protease_label in protease_labels:
            ...
            for protease_pattern in protease_patterns[protease]:
                ...
                protease_pattern_matrix = sequence.vectorize_pattern(
                ...
                try:
                    protease_constituent_pattern = extraction.get_pattern_constituents(
...
def generate_protease_pattern_frequency_matrix(
        patterns,
        protease_patterns,
        protease_labels
    ):
...
def generate_protease_pattern_percentage_frequency_matrix(
...
def calculate_pattern_frequency(pattern):
    ...
    try:
        constituent_pattern = extraction.get_pattern_constituents(
            pattern.pattern_matrix,
            pattern.background
        )
    except AttributeError:
...
def generate_sequence_clustermap(
        title,
        pattern_similarity_matrix,
        output_path,
        row_colors=None,
    ...
    clustermap.figure.suptitle(title)
...
def generate_protease_pattern_heatmap(
        title,
        patterns,
        scoring_matrix,
        protease_labels,
    ...
    sns.heatmap(
        ...
        vmin=0.0,
        vmax=(
            100.0 if (np.amax(scoring_matrix).astype(np.float64) == 0)
                else (np.amax(scoring_matrix).astype(np.float64))
        )
    )
    
    # Dynamic label coloring.
    protease_pattern_scores = np.array(scoring_matrix, dtype=np.float64)
