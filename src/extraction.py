...
class Pattern:
        ...
        if current_depth < max_depth:
            while self.subset_tensor.shape[0] >= minimum_occurrences:
                ...
                removed_positional_residue_mask = np.invert(
                    self.removed_positional_residues.astype(bool)
                )
                binomial_p_values = (
                ...
                """
    def calculate_position_specific_background_frequencies(
            self,
            allow_compound_residue_decomposition
        ):
        ...
        # Light-memory, slow version.
           ...
           #  Get subset of background matching pattern.
            ...
            for position, residues in character_pattern.items(): # pandas > 2.0
                if residues == '.':
                    continue
            ...
            for position, residues in fixed_downstream_characters.items():
                if residues == '.':
                    continue
                stripped_residues = residues.replace('[', '').replace(']', '')
                background_df = background_df[
    ...
    def fix_homogeneous_positional_residues(self):
        ...
        homogeneous_positional_residues[
            homogeneous_positional_residues!=self.subset_tensor.shape[0]
        ] = 0.0
        homogeneous_positional_residues[
            homogeneous_positional_residues==self.subset_tensor.shape[0]
        ] = 1.0
    ...
    def _adjust_positional_background_frequencies(self):
        ...
        positional_background_frequencies = (
            positional_background_frequencies
            * np.invert(fixed_positions.astype(bool)).astype(np.float64)
        )
    ...
    def _compound_residue_background_frequencies(
            self,
            compound_residues,
    ...
    def _calculate_background_frequencies(
            self, 
            removed_positional_residues
        ):
        """
    ...
    def calculate_binomial_p_values(
            self,
            positional_residue_frequencies,
            positional_background_frequencies
        ):
    ...
    def calculate_positional_weights(
            self,
            positional_residue_frequencies,
            positional_residue_fold_change,
            positional_background_frequencies
    ...
    def _remove_compound_residue_constituents(
            self,
            residue_coordinates,
            new_pattern_removed_positional_residues
        ):
    ...
    # TODO: Implement sequence logo map generation. (weblogo)
    def generate_logo_map(self, output_directory):
        ...
        colorscheme = weblogo.ColorScheme(baserules, alphabet = logo_alphabet)

        seqs = weblogo.read_seq_data(io.StringIO(fasta), alphabet=logo_alphabet)
...
class PatternContainer:
    ...
    def generate_pattern_outputs(self, logo_maps=True):
        ...
        for pattern in self.pattern_list:
            ...
            try:
                pattern.constituent_pattern = get_pattern_constituents(
                    pattern.pattern_matrix,
                    self.background
                )
            except AttributeError:
